...
public final class MultiDexExtractor implements Closeable {
    ...
    private static final String TAG = MultiDexOffline.TAG;
     ...
     *                     secondary dex files
     */
    List<? extends File> load(SharedPreferences prefs, String prefsKeyPrefix, boolean forceReload)
            throws IOException {
        ...
        List<MultiDexExtractor.ExtractedDex> files;
        if (!forceReload && !isModified(prefs, sourceApk, sourceCrc, prefsKeyPrefix)) {
            try {
                files = loadExistingExtractions(prefs, prefsKeyPrefix);
            } catch (IOException ioe) {
                Log.w(TAG, "Failed to reload existing extracted secondary dex files,"
                        + " falling back to fresh extraction", ioe);
                files = performExtractions();
                putStoredApkInfo(prefs, prefsKeyPrefix, getTimeStamp(sourceApk), sourceCrc,
                        files);
            }
        } else {
            if (forceReload) {
     ...
     * {@link #LOCK_FILENAME}.
     */
    private List<MultiDexExtractor.ExtractedDex> loadExistingExtractions(
        ...
        final List<MultiDexExtractor.ExtractedDex> files = new ArrayList<>(totalDexNumber - 1);
    ...
    private static void putStoredApkInfo(SharedPreferences prefs, String keyPrefix, long timeStamp,
                                         long crc, List<MultiDexExtractor.ExtractedDex> extractedDexes) {
        ...
        for (MultiDexExtractor.ExtractedDex dex : extractedDexes) {
